...
echo_pin=19      # the GPIO pin that indicates a returning ultrasonic wave when it is set to high (input)
...
def check_distance():
    ...
    while len(samples_list) < number_of_samples:  # Checks if the samples_list contains the required number_of_samples
        ...
        if len(stack) == 2:  # Stack has two elements on it.
            ...
            time.sleep(
                sample_sleep)  # Pause to make sure we don't overload the sensor with requests and allow the noise to die down
...
GPIO.add_event_detect(echo_pin, GPIO.BOTH,
                          callback=timer_call)  # add rising and falling edge detection on echo_pin (input)

rounded_reading = round(check_distance(), 1)
def main():
    while True:

        if rounded_reading>45:
            move_servo1()
            move_servo2()
        else:
            break
